...
// 先頭の数字（例: "02_Yamazaki..." -> 2）でソートする。投稿順の想定
...
// ストーリーズ・リールとも同じ動画ファイルを使う（静止画は使わない構成）
async function postStory(igUserId, videoFile) {
  const video_url = rawUrlFor("videos", videoFile);
  return createAndPublish(
    igUserId,
    "story",
    { video_url, media_type: "STORIES" },
    { maxTries: 20, intervalMs: 5000 }
  );
}
...
async function main() {
  ...
  if (candidates.length === 0) {
    console.log("投稿可能な在庫（動画+キャプションが揃った未投稿分）がありません。今回はスキップします。");
    return;
  }
  ...
  if (!ledger[base].story) {
    try {
      const mediaId = await postStory(igUserId, videoFile);
  ...
  // 投稿が完了した分は在庫から削除し、二重投稿を防ぐ
  const storyDone = !!ledger[base].story;
  const reelDone = !!ledger[base].reel;
  if (storyDone && reelDone) {
    deleteIfExists(path.join(VIDEOS_DIR, videoFile));
